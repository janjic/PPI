...
class LocationController extends BaseAdminController
{
    ...
    public function createAutoSearchQueryBuilder(array $entityConfig, $searchQuery, $sortField = null, $sortDirection = null, $dqlFilter = null)
    {
        ...
        if (($this->forEntity === (new \ReflectionClass(Scene::class))->getShortName()) && ($parentId = $this->request->get('parent'))) {
            $queryBuilder->join('entity.parent', 'parent');
            $queryBuilder->andwhere("parent.id = :pid");
            $queryParameters['pid'] = (int)$parentId;
        } else {
            $queryBuilder->andwhere("entity.lvl = 0");
        }
